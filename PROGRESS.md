...
- **#51 페이즈 업적 + 검증 헬퍼** — achievements.js: 6 업적 신규 (`phase_excavation`/`foundation`/`lifting`/`envelope`/`mep` 4언어 + `tutorial_complete` 🎓). hud.js `_onPhaseChange` 가 fromPhase id 로 PHASE_ACH 매핑 unlock + isFinal 시 tutorial_complete unlock. phase_controller.js: `__bulsa.fastForward(n)` 강제 advance 헬퍼 (force=true 로 canAdvance 우회, 5 페이즈 자동 진행 후 dump 반환).
